...
$oldDir = $PWD
Set-Location F:\Powershell\PnP-UserProfile
...
$user = Get-ADUser salima.moorji -properties "SID"

$spUser = Get-PnPUser | Where-Object Email -eq "salima.moorji@gov.ab.ca"
If ($spUser.UserId.NameId -ne $user.SID.Value) {
    Write-Host "SID does not match"
}

Write-Host $spUser.UserId.NameId
